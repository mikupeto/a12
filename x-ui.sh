...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/mikupeto/a12/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    if [[ $yn =~ "Y"|"y" ]]; then
        ...
        if [[ -z "$last_version" ]]; then
            ...
            last_version=$(curl -sm8 https://raw.githubusercontent.com/mikupeto/a12/main/config/version)
            if [[ -z "$last_version" ]]; then
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/mikupeto/a12/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
